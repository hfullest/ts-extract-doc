...
export const generateTable = (
  propDocs: Record<string, DocumentProp | DocumentMethod | DocumentParameter>,
  document: Document,
  config: TableConfig,
  options: { header?: string; css?: string; footer?: string },
) => {
  const propsDoc = Object.entries(propDocs ?? {});
  if (!propsDoc.length) return '';
    ...
    .map(([, doc], index) => {
      ...
      const fields = columns
        .map((col) => {
          ...
          return `<td data-type='${col?.dataIndex}' ${style}>${content.current ?? config?.whiteSpaceFill}</td>`;
        })
        .join('\n');
      return `<tr>${fields ? `\n${fields}\n` : ''}</tr>`;
    })
  ...
  const presetCss = options?.css ?? `<style>table[data-id='${fullId}'] p { margin:0; }</style>`;
  const tableContent = [presetCss, headerRows, bodyRows].filter(Boolean).join('\n');
  const table = columns?.length && propsDoc?.length ? `<table data-id='${fullId}'>\n${tableContent}\n</table>\n` : '';
