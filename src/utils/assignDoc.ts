...
import { Document, DocumentProp } from '../interface';

export type CommonDeclarationNode =
  | TypeAliasDeclaration
  | InterfaceDeclaration
  | EnumDeclaration
  | ModuleDeclaration
...
export const assignCommonDoc = (symbol: Symbol): Document => {
  const node = symbol?.getDeclarations()[0] as CommonDeclarationNode;
